...
def write_synthetic_dataset() -> None:
    """Write a small, well-separated synthetic creditcard-schema CSV."""
    rng = np.random.default_rng(7)
    n, n_fraud = 4000, 80
    labels = np.r_[np.zeros(n - n_fraud, int), np.ones(n_fraud, int)]
    rng.shuffle(labels)
    ...
    print(f"wrote {dest} ({n} rows, {n_fraud} frauds)", flush=True)
