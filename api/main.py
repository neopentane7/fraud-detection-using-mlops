...
from src.config import FEATURE_COLUMNS, SCALED_COLUMNS, SCALER_PATH, THRESHOLD_PATH

logging.basicConfig(level=logging.INFO, format="[%(levelname)s] %(name)s: %(message)s")
logger = logging.getLogger(__name__)

HIGH_RISK_PROBABILITY = 0.80  # above this, attach SHAP explanation if enabled
...
def _load_threshold(path: Path = THRESHOLD_PATH) -> float:
    """Load the decision threshold persisted by the training stage."""
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    try:
        STATE["model"] = mlflow.pyfunc.load_model(f"models:/{model_name}/{model_stage}")
        ...
        logger.info("Loaded model %s/%s (v%s)", model_name, model_stage, version)
...
async def add_request_id(request: Request, call_next: CallNext) -> Response:
    """Attach an X-Request-ID to every response for traceability."""
...
async def predict(
    ...
    request_id = request.headers.get("X-Request-ID", str(uuid.uuid4()))
    _audit_log(request_id, _hash_payload(frame), result)
