...
design decision: it's why accuracy is banned as a metric, why we use
`scale_pos_weight`, and why a single holdout split is statistically noisy
(only ~71–95 frauds land in each test fold).
...
AUPRC (~0.83) is the honest summary under imbalance — far more informative than
ROC-AUC here because it ignores the huge true-negative mass. The red dot is the
deployed operating point chosen by the recall-first strategy.
...
is the metric we actually reason about.
...
explicit and is the best thing to walk an interviewer through.
...
improving ranking, so we tuned it down to 24. **The threshold strategy, not
`scale_pos_weight`, was the real lever.**
